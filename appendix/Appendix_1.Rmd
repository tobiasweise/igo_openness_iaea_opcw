---
title: Explaining the Variation in the Opennesss of Intergovernmental Arms Control
...
data %>% filter(IO == "OPCW") %>% 
  select(ACT.Part.1, ACT.Part.2, ACT.Part.3) %>%
	cor(use = 'pairwise.complete.obs')
```
All measures of NGOs and NGO representatives correlates highly. 
The presence of new NGOs, however, seems to follow a different 
...
I added this to the usual public information costs, as website etc. provides 
...
# loaded with R, quandl only has data until 1969
...
things. Consequently, I will use both measures for the analysis.
...
#members_iaea$isoc[which(members_iaea$value == 
#					"Socialist Federal Rep. of Yugoslavia")] <- "YUG"
...
# OPCW membership data is not available in other sources, I thus 
# create the data here from alternative sources
# take data from membership table off OPCW website
...
# add non-matching values manually
#members_opcw$isoc[which(members_opcw$country ==
...
## NB-Press Salience
### IAEA
-  Shows how present the organization is in the global press.
